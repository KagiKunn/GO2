...
public class UnitGameManagerA : MonoBehaviour {
	...
	public Dictionary<UnitData, int> unitPlacementStatus; // 0: 미배치, 1: 왼쪽 성벽, 2: 오른쪽 성벽 -UnitData>string
	...
	public void SaveSlotUnitData(List<SlotUnitData> dataList) {
		CustomLogger.Log("save 어쩌고 들옴 " + dataList);

		CustomLogger.Log(dataList.Count);
		...
		if (dataList.Count == 0) {
			CustomLogger.LogWarning("No data to save.");
	...
	public List<SlotUnitData> LoadSlotUnitData() {
		...
		if (File.Exists(filePath)) {
			...
			CustomLogger.LogWarning("저장된 유닛 데이터 파일을 찾을 수 없습니다.");
