		...
		for (var j = 0; j < _prefabUnitList.Count; j++) {
			...
			for (var i = 0; i < ttObjST._fullSetList.Count; i++) {
				...
				if (ttObjST._fullSetList[i].sprite != null) {
					...
					ttObjST._fullSetList[i].rectTransform.pivot = new Vector2(ttObjST._fullSetList[i].sprite.pivot.x / ttObjST._fullSetList[i].sprite.rect.width,
						ttObjST._fullSetList[i].sprite.pivot.y / ttObjST._fullSetList[i].sprite.rect.height);
