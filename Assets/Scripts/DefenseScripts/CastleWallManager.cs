...
public class CastleWallManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // DestroyedWalls를 클래스 멤버로 초기화하고 비활성화
        destroyedWalls = GameObject.FindGameObjectsWithTag("DestroyedWalls");
        foreach (GameObject wall in destroyedWalls)
        {
            wall.SetActive(false); // 오브젝트를 비활성화하여 보이지 않게 합니다.
        }
    }
    ...
    private void ShowDestroyedWalls()
    {
        foreach (GameObject wall in destroyedWalls)
        {
            if (wall != null)
            {
                wall.SetActive(true); // 오브젝트를 활성화하여 보이게 합니다.
            }
        }
    }
